...
默认配置面向两张 NVIDIA A40 48GB，采用全参数 BF16 训练，并集成
vLLM rollout、FSDP、规则奖励、WandB、本地日志、GPU 空闲等待和训练前后
回答对比。
...
等待脚本会监控 GPU 显存和利用率。当同样的两张 GPU 连续三次满足空闲条件
后，自动启动训练。
